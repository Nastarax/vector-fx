...
def _build_row(ccy, ind, te_history, investing_cpi, investing_ppi,
    ...
    # CPI, sPMI and Household Spending are the exceptions: per request they fall
    # back to Actual vs Previous when no forecast is published, so their chips
    # match the heatmap cells (the Forecast column still shows n/a; only the
    # impact benchmark falls back).
    benchmark = forecast
    if ind_id in ("cpi", "spmi", "household_spending") and benchmark is None:
        benchmark = previous
